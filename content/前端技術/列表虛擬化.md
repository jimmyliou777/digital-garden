...
**TL;DR：** @tanstack/react-virtual 是一個 headless 虛擬捲軸 hook，只渲染可見區域的 DOM 節點，讓萬筆資料清單維持 60 FPS。支援 Row / Column / Grid 三種模式、動態高度量測、跨框架共用核心。

> 圖表工具用法見 [[在 Quartz Blog 中使用 Mermaid 與 Excalidraw 圖表|Mermaid 與 Excalidraw 圖表]]。

```mermaid
flowchart LR
    A["📜 完整資料<br>10,000 筆"] --> B["📐 計算可見範圍<br>+ overscan"]
    B --> C["🖥️ 只渲染 ~20 個 DOM"]
    C --> D["⚡ 維持 60 FPS"]
```
...
TanStack Virtual 著重「核心邏輯最小化 + Adapter 包裝」，因此更新頻繁、社群活躍，並已針對 React 19 基於 useSyncExternalStore 的變更修正相容性問題。同類選型決策表體裁可參考 [[微前端組件化方案指標評比]]。
