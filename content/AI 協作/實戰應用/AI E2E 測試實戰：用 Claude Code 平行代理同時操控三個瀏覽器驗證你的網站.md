...
**TL;DR：** Claude Code 的 Agent tool 可以平行派發多個子代理，每個代理各自啟動獨立的 Chromium 瀏覽器執行 Playwright 腳本。實測 3 個 Agent 同時驗證 blog 的搜尋功能、元件渲染、Dark Mode 與 RWD——97 秒完成全部測試，比循序執行快 63%。

> **讀者假設：** 你已經在使用 Claude Code，對 Agent tool、Skill、MCP 這些概念有基本認識。如果還不熟悉平行代理的派發邏輯，這篇會從頭解釋。
